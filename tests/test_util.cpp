...
#if defined(QMP_COMMS)
  printf("This code built with QMP-Comms\n");
...
#elif defined(MPI_COMMS)
  printf("This code built with MPI-Comms\n");
  MPI_Init(&argc, &argv);
#endif
...
void usage(char** argv )
{
  ...
  printf("    --compute-plaquette                       # Whether to compute plaquette in the test after loading gauge field (true/false, default true)\n");
...
int process_command_line_option(int argc, char** argv, int* idx)
{
  ...
  if( strcmp(argv[i], "--compute-plaquette") == 0){
    if (i+1 >= argc){
      usage(argv);
    }
    if (strcmp(argv[i+1], "true") == 0){
...
/* C. Kallidonis
 * Code for reading a Lime gauge file
 * Requires liblime.a
 * Original code by Kyriakos Hadjiyiannakou!
 */
...
template <typename Float>
static void applyGaugeFieldScaling_KH(Float **gauge, int Vh, QudaGaugeParam *param) {
  // Apply spatial scaling factor (u0) to spatial links
  for (int d = 0; d < 3; d++) {
    for (int i = 0; i < gaugeSiteSize*Vh*2; i++) {
      gauge[d][i] /= param->anisotropy;
...
void applyBoundaryCondition(void **gauge, int Vh ,QudaGaugeParam *gauge_param){
  ...
  if( gauge_param->cpu_prec == QUDA_DOUBLE_PRECISION )
    applyGaugeFieldScaling_KH<double>((double**)gauge, Vh, gauge_param); 
  else
